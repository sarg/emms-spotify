...
(defmacro consult-spotify-emms--secret-value (name)
  "Extract secret NAME according to its type."
  `(cond
    ((functionp ,name) (funcall ,name))
    ((characterp ,name) consult-spotify-emms-client-id)
    (t (user-error ,(concat (symbol-name name) " is not set")))))

(declare-function consult-spotify-emms--auth-token "consult-spotify-emms")
(let ((token))
  (defun consult-spotify-emms--auth-token (&optional force)
    (when (or (null token) force)
      (request "https://accounts.spotify.com/api/token"
        ...
        :success (cl-function
                  (lambda (&key data &allow-other-keys)
                    (setq token (alist-get 'access_token data))))
        :error (cl-function
...
(defun consult-spotify-emms--api-request (path &rest args)
  "Make a request to spotify api endpoint at PATH. Add ARGS to request call."
  (declare (indent 1))
  (let ((retries 0) result)
    (while (and (< retries 2) (not result))
      (apply #'request (concat "https://api.spotify.com/v1" path)
             :headers `(("Authorization" . ,(concat "Bearer " (consult-spotify-emms--auth-token))))
             :sync t
      ...
      (when (eq result 'unauthorized)
        ...
        (consult-spotify-emms--auth-token 'force)))
    result))
...
(defun consult-spotify-emms--search (input &optional type)
  "Search itsm of TYPE (albums, playlists, tracks) given user INPUT."
  (declare (indent 1))
  (thread-last
